ENV_DEPLOY=cert.staging.eu01.hastens.demandware.net
echo "#######################"
...
echo "       deploy          "
echo "#######################"
sfcc-ci client:auth $OCAPI_CLIENT_ID_STG $OCAPI_SECRET_STG --renew
...
for dir in ./* ;do
    if [[ -d $dir ]];then
        ...
        then
            ...
            sfcc-ci code:deploy ${CODE_VERSION}.zip -i $ENV_DEPLOY -c ../CI/deploy.p12 -p $DEPLOY_CERT_PASSPHRASE
