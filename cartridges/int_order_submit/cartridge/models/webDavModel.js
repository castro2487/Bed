...
webDavInstance.prototype = {
    ...
    */
    ...
    /* get directory list from webdav
    * fullPathFiles = path of the folder that contains the directories
    * rootDirectory = impex, etc...
    * directoryName = necessary if only root directory is libraries or catalogs
    */
    getDirectoryList: function (fullPathDirectories, rootDirectory, directoryName) {
        try {
            var result = {};

            if (empty(rootDirectory) || empty(fullPathDirectories)) {
                Logger.info('Root Directory and Full Path Files are mandatory.');
                result.success = false;
                return result;
            }
            ...
            var filesPath = new File(rootDir.concat(Separator, fullPathDirectories, Separator));
            ...
            var directories = filesPath.listFiles();

            var arrayDirectories = [];
            for (var i = 0; i < directories.length; i++) {
                if (directories[i].isDirectory()) {
                    arrayDirectories.push(directories[i]);
                }
            }

            if (empty(directories) || empty(arrayDirectories)) {
                Logger.info('No directories in directory {0}.', fullPathDirectories);
                result.success = false;
                return result;
            }

            result.directoryList = arrayDirectories;
            result.success = true;
