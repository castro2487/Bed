...
interface PartnerSliderContent {
    store: {
        slider: {
            'hide-nav': string;
...
export function PartnerSlider() {
    const { content: { store: { slider } } } = useServer<PartnerSliderContent>();
    ...
    return (
        <div className="has-partner-slides-show">
            <Slider
                ...
                infiniteLoop
                showIndicators
                slides={JSON.parse(slider?.['is-randomized'] || 'false') ? randomize() : slidesArray}
            />
        </div>
    );
}
