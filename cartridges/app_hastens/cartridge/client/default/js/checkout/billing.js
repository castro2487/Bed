    ...
    if ($billingAddressSelector && $billingAddressSelector.length === 1) {
        ...
        if (customer.addresses && customer.addresses.length > 0) {
            $billingAddressSelector.append(addressHelpers.methods.optionValueForAddress(
                ...
                var currentCountry = $billingAddressSelector.data('current-country');
            customer.addresses.forEach(function (address) {
                var isSelected = order.billing.matchingAddressId === address.ID;
                hasSelectedAddress = hasSelectedAddress || isSelected;
                // Customer Address option
                if (address.countryCode.value === currentCountry) {
                    $billingAddressSelector.append(
...
function updateBillingAddressFormValues(order) {
    ...
    var prefixCountry = billing.billingAddress.address.prefixCountry;
    var prefix = billing.billingAddress.address.prefix;

    // current locale prefix
    var selector = $('#billingAddressSelector');
    var currentLocalePrefix = selector.data('new-prefix');
    var currentLocaleCountry = selector.data('new-prefix-country');
    
    // if no prefix, use the one associated with the current locale
    $('input[name$=_prefixCountry]', form).val(prefixCountry && prefixCountry.length > 0 ? prefixCountry : currentLocaleCountry);
    $('input[name$=_prefix]', form).val(prefix && prefix.length > 0 ? prefix : currentLocalePrefix);

    // remove old country flag from prefix selector
    var flag = $('.current-flag', form);
    flag.removeClass();
    // add new country flag in prefix selector
    var newCountryCode = $('input[name$=_prefixCountry]', form).val();
    flag.addClass('current-flag flag-icon flag-icon-' + newCountryCode.toLowerCase());
...
/**
 * clears the billing address form values except for email and phone fields
 * (since they are provided by shipping form)
 */
function clearBillingAddressFormValues($form) {
    // current locale prefix
    var selector = $('#billingAddressSelector');
    var prefix = selector.data('new-prefix');
    var prefixCountry = selector.data('new-prefix-country');

    // update prefix selector flag
    var currentFlag = $(".current-flag", $form);
    currentFlag.removeClass();
    currentFlag.addClass('current-flag flag-icon flag-icon-' + prefixCountry.toLowerCase());

    var email = $('input[name$=_email]', $form).val();
...
function updateBillingInformation(order, customer) {
    ...
    if (order.billing.billingAddress.address) {
        var prefix = order.billing.billingAddress.address.prefix ? order.billing.billingAddress.address.prefix + ' ' : '';
        $('.order-summary-phone').text(prefix + order.billing.billingAddress.address.phone);
    }
}
...
module.exports = {
    ...
    selectBillingAddress: function () {
        $('.payment-form .addressSelector').on('change', function () {
            ...
            if(!$('[name$=prefixCountry]', form).val()) {
                $('[name$=prefixCountry]', form).val(attrs['prefixcountry']);
            }
            if($('[name$=prefix]', form).val().length === 0) {
                var selector = $('.single-shipping .addressSelector');
                var currentLocalePrefix = selector.data('new-prefix');
                var currentLocaleCountry = selector.data('new-prefix-country');  
                $('input[name$=_prefixCountry]', form).val(currentLocaleCountry);
                $('input[name$=_prefix]', form).val(currentLocalePrefix);
            }
            // remove old country flag from prefix selector
            var flag = $('.current-flag', form);
            flag.removeClass();
    ...
    clearBillingForm: function () {
        $('body').on('checkout:clearBillingForm', function (e, $data) {
            clearBillingAddressFormValues($data);
        });
    },
