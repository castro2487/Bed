...
$('#dwfrm_billing').submit(function (e) {
  ...
  $.ajax({
    type: 'POST',
    url: url,
    data: serializedForm.replace('dwfrm_billing_paymentMethod=&', '&'),
    async: false,
 ...
 */
 ...
 function displaySelectedMethod(type) {
  ...
  if (['paywithgoogle', 'mbway'].indexOf(type) > -1) {
    document.querySelector('button[value="submit-payment"]').disabled = true;
  } else {
    document.querySelector('button[value="submit-payment"]').disabled = false;
    $('.paypal-container #component_paypal')[0].style.zIndex = type === 'paypal' ? 0 : -1;

    if ($('#paymentMethodsList').data('summary-less-payment-methods').indexOf(type) > -1) {
      $('.btn-secondary.submit-payment').addClass('d-none');
      $('.submit-payment-card').removeClass('d-none');
      $('#isTermsAndConditionsSubmitPayment').trigger('change');
    } else {
      $('.submit-payment-card').addClass('d-none');
      $('.btn-secondary.submit-payment').removeClass('d-none');
    }
  }
...
function _renderGenericComponent() {
  _renderGenericComponent = _asyncToGenerator(function* () {
    ...
    $('.paypal-container #component_paypal').empty();
    ...
    getPaymentMethods(function (data) {
      ...
      data.AdyenPaymentMethods.paymentMethods.forEach((pm, i) => { 
        !isMethodTypeBlocked(pm.type) && renderPaymentMethod(pm, false, data.ImagePath, data.AdyenDescriptions[i].description, i);
      });
...
function renderPaymentMethod(paymentMethod, storedPaymentMethodBool, path, AdyenDescriptions, index) {
  var description = AdyenDescriptions || '';
  var node;
  ...
  var liContents = "\n                              <div class=\"custom-control custom-radio custom-radio-payment\">   <input name=\"brandCode\" class=\"custom-control-input\" type=\"radio\" value=\"".concat(paymentMethodID, "\" id=\"rb_").concat(paymentMethodID, "\">\n             <label id=\"lb_").concat(paymentMethodID, "\" class=\"custom-control-label\" for=\"rb_").concat(paymentMethodID, "\">").concat(label, "</label>\n              <img class=\"paymentMethod_img\" src=\"").concat(imagePath, "\" ></img>\n  </div>             ");


  if (description && AdyenDescriptions !== 'PayPal') {
    liContents += "<p class=\"ideal-description\">".concat(description, "</p>");
  }
  ...
  if (index === 0) {
    li.classList.add('active-radio-button');
  }
  ...
  // paypal button will be placed under the Next Step button
  if (paymentMethod.type === 'paypal') {
    container = $('.paypal-container #component_paypal')[0];
  }
  ...
  input.onchange = event => {
    $('.paymentMethod').removeClass('active-radio-button');
    $(input).parents('.paymentMethod').addClass('active-radio-button');
