...
function getConfig(apiProduct, params) {
    ...
    if (variationModel) {
        if(variationModel.selectedVariant) {
            //color and size are selected - return the sku selected
            apiProduct = variationModel.selectedVariant;
        } else {
            //only color is selected - loop on all variation group in order to find the selected one
            if(variables && variables.color && variationModel.variationGroups && variationModel.variationGroups.length > 0) {
                var selectedColor = variables.color.value;
                var selectedVariationGroup;
                for(var i = 0; i < variationModel.variationGroups.length; i++) {
                    var variationColor = variationModel.getVariationValue(variationModel.variationGroups[i], variationModel.productVariationAttributes[1]);
                    if(variationColor && variationColor.value === selectedColor) {
                        selectedVariationGroup = variationModel.variationGroups[i];
                        break;
                    }
                }    
                if(selectedVariationGroup) {
                    apiProduct = selectedVariationGroup;
                }
            }
        }
    }
...
function showProductPage(querystring, reqPageMetaData) {
    ...
    // get master url if product type is variant
    if(product.productType ===  'master') {
        ...
        var masterProduct = apiProduct.getMasterProduct();
        var canonicalUrl = URLUtils.url('Product-Show', 'pid', masterProduct.ID);
    }
