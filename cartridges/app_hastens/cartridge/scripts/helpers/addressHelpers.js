...
function checkIfAddressStored(address, storedAddresses) {
    for (var i = 0, l = storedAddresses.length; i < l; i++) {
        if (storedAddresses[i].address1 === address.address1
            ...
            || storedAddresses[i].ID === generateAddressName(address)) {
            return true;
        }
    }
    return false;
...
function updateAddressFields(newAddress, address) {
    ...
    newAddress.custom.prefix = address.prefix || ''; 
    newAddress.custom.prefixCountry = address.prefixCountry || '';
    newAddress.custom.prefixCountry = newAddress.custom.prefixCountry.toUpperCase();
    var phone = address.phone || ''
    // contcatenate prefix to phone if not already done
    if (phone && phone[0] !== '+') {
        phone = newAddress.custom.prefix + phone;
    }
 ...
 */
 function copyShippingAddress(address) {
    return {
        ...
        prefix: address.custom.prefix,
        prefixCountry: address.custom.prefixCountry,
        phone: address.phone,
