...
// utility
var murmurhash = require('*/cartridge/scripts/murmurhash');
...
function calculateTax(basket, orderNo) {
	var previousAvataxError = session.custom.avataxError;
	session.custom.avataxError = false;
	...
	try {
		...
		transactionModel.aliShipTo = aliShipTo;
		...
		if (session.privacy.avataxtransactionmodel && session.privacy.avataxtransactionmodel === hash && !previousAvataxError) {
		...
		Transaction.wrap(function () {
		    basket.custom.ATZeroTaxError = true;
		});
		...
		if (orderNo) {
			...
			if (orderObj) {
				Transaction.wrap(
					function () {
						orderObj.custom.ATInvoiceMessage = null;
						orderObj.custom.ATTaxDetail = null;
						orderObj.custom.ATLandedCost = null;
						orderObj.custom.ATCustomsDuty = null;
						orderObj.custom.ATGenericMessage = null;
						orderObj.custom.ATTax = null;
					}
				);
			}
		} else if (basket) {
			Transaction.wrap(
				function () {
					basket.custom.ATInvoiceMessage = null;
					basket.custom.ATTaxDetail = null;
					basket.custom.ATLandedCost = null;
					basket.custom.ATCustomsDuty = null;
					basket.custom.ATGenericMessage = null;
					basket.custom.ATTax = null;
				}
			);
		}
		...
		if (svcResponse.statusCode === 'ERROR') {
			...
			session.custom.avataxError = true;
			return {
				OK: true
			};
		}
		...
		if (!svcResponse.statusCode) {
			Transaction.wrap(function () {
				basket.custom.ATZeroTaxError = false;
			});
