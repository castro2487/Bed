...
function prependPlaceOrder(req, res, next) {
    ...
    if (req.currentCustomer.profile) {
        ...
        var allAddresses = addressHelpers.gatherShippingAddresses(currentBasket);
    ...
    var paymentInstrument = order.getPaymentInstruments(constants.METHOD_ADYEN_COMPONENT)[0]; // Handles payment authorization
    ...
    if (handlePaymentResult.threeDS2) {
      ...
      res.json({
        error: false,
        continueUrl: URLUtils.url('Adyen-Adyen3DS2', 'resultCode', handlePaymentResult.resultCode, 'merchantReference', order.orderNo, 'orderToken', orderToken).toString()
      });
      this.emit('route:Complete', req, res);
      return;
    }
    ...
    if (handlePaymentResult.redirectObject) {
      ...
      if (handlePaymentResult.authorized3d) {
        ...
        res.json({
          error: false,
          continueUrl: URLUtils.url('Adyen-Adyen3D', 'IssuerURL', handlePaymentResult.redirectObject.url, 'PaRequest', handlePaymentResult.redirectObject.data.PaReq, 'MD', handlePaymentResult.redirectObject.data.MD, 'signature', handlePaymentResult.signature, 'merchantReference', order.orderNo, 'orderToken', orderToken).toString()
        });
        this.emit('route:Complete', req, res);
        return;
      }
      ...
      res.json({
        error: false,
        continueUrl: URLUtils.url('Adyen-Redirect', 'redirectUrl', handlePaymentResult.redirectObject.url, 'signature', handlePaymentResult.signature, 'merchantReference', order.orderNo, 'orderToken', orderToken).toString()
      });
      this.emit('route:Complete', req, res);
      return;
    }
