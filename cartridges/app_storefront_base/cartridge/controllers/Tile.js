...
server.get('Show', cache.applyPromotionSensitiveCache, function (req, res, next) {
    ...
    try {
        ...
        var color = product.firstRepresentedProduct.custom.color;

        var firstVariationProduct = ProductFactory.get({
                pid: product.id,
                variables: {
                    color: {
                        id: product.id,
        ...
        productUrl = URLUtils.url(
            ...
            'pid',
            product.id,
            'dwvar_' + product.id.replace(/_/g, '__') + '_color',
            color
        ).toString();
    ...
    var context = {
        ...
        display: {},
        firstVariationProduct: firstVariationProduct
    };
