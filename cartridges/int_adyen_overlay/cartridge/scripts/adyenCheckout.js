...
function createPaymentRequest(args) {
  try {
    ...
    var paymentMethodType = paymentRequest.paymentMethod.type; // Create billing and delivery address objects for new orders, no address fields for credit cards through My Account
  ...
  try {
    ...
    if (paymentResponse.resultCode === 'IdentifyShopper' || paymentResponse.resultCode === 'ChallengeShopper') {
      if (responseObject.action) {
        paymentResponse.action = JSON.stringify(responseObject.action);
      }
