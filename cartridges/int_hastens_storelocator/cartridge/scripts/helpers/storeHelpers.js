...
function getStores(radius, postalCode, lat, long, geolocation, showMap, url) {
    ...
    if (postalCode && postalCode !== '') {
        ...
        // Limit search to the given country
        storeMgrResult = StoreMgr.searchStoresByCoordinates(
            location.lat, location.long, distanceUnit, resolvedRadius,
 ...
 * @returns {Object}
 */
 function getStoreAddress(storeObject) {
    return {
        address1: storeObject.address1,
        address2: storeObject.address2,
        city: storeObject.city,
        stateCode: storeObject.stateCode,
        postalCode: storeObject.postalCode,
        countryCode: storeObject.countryCode.value,
        displayAddress: storeObject.custom['display-address'],
    };
}
